...
class _SeriesOfComicWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var seriesOfComic = model?.seriesOfComics;
    if (seriesOfComic == null || model == null) return const SizedBox.shrink();
    return ListView.builder(
      // itemCount: model.seriesOfComics.length,
      itemExtent: 120,
      scrollDirection: Axis.horizontal,
      itemBuilder: (BuildContext context, int index) {
        model.showSeriesOfComicByIndex(index);
        final comicOfSeries = model.seriesOfComics[index];
